...
class UpgradePatch extends Component {
  postSynthesize() {
    ...
    upgradeWorkflow.addOverride('jobs.pr.steps.5', {
      ...
      run: 'curl -X POST -H "Accept: application/vnd.github.v3+json" -H "Authorization: token ${GITHUB_TOKEN}" https://api.github.com/repos/${{ github.repository }}/actions/workflows/build/dispatches -d "{\\"ref\\":\\"$(git rev-parse HEAD)\\"}"',
    });
